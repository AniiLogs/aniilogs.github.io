...
const LOCAL_COMPLETION_STORAGE_KEY = "aniilogs:explorer:completed:v1";
const LOCAL_PREFERENCES_STORAGE_KEY = "aniilogs:explorer:preferences:v1";
...
const API_URL = String(
  SITE_CONFIG.apiUrl
  || SITE_CONFIG.shareApiUrl
  || (window.location.hostname === "aniilogs.github.io" ? "" : `${window.location.origin}/api`),
).replace(/\/+$/, "");
const SHARE_API_URL = API_URL;
...
const CATALOG_ROLE_META = Object.freeze({
  DPS: { slug: "dps", color: "#e76561" },
  REGEN: { slug: "regen", color: "#58b7d9" },
  BREAK: { slug: "break", color: "#ae7ee8" },
  HEALER: { slug: "healer", color: "#65c889" },
  SUPPORT: { slug: "support", color: "#e47fae" },
  DEFENSE: { slug: "defense", color: "#d2aa54" },
});
const CATALOG_ELEMENT_META = Object.freeze({
  fire: { slug: "fire", color: "#e03d3d" },
  grass: { slug: "grass", color: "#43a061" },
  water: { slug: "water", color: "#1e90ff" },
  rock: { slug: "rock", color: "#b8a278" },
  earth: { slug: "rock", color: "#b8a278" },
  electric: { slug: "electric", color: "#e2c10b" },
  electricity: { slug: "electric", color: "#e2c10b" },
  ice: { slug: "ice", color: "#58d0e8" },
  wind: { slug: "wind", color: "#58c7b1" },
  dark: { slug: "dark", color: "#7b4ca9" },
  shadow: { slug: "dark", color: "#7b4ca9" },
  holy: { slug: "holy", label: "Light", color: "#f6a93c" },
  light: { slug: "holy", label: "Light", color: "#f6a93c" },
  metal: { slug: "metal", color: "#708c9c" },
  poison: { slug: "poison", color: "#8d63b8" },
  psychic: { slug: "psychic", color: "#ef65b0" },
  psychokinesis: { slug: "psychic", color: "#ef65b0" },
  normal: { slug: "normal", color: "#9faaad" },
});
const CATALOG_HOMELAND_META = Object.freeze({
  ...
  1100: { slug: "carry", color: "#6285cc" },
  1101: { slug: "artisanship", color: "#71b258" },
  1102: { slug: "leisure", color: "#e77894" },
  1103: { slug: "incense-making", color: "#b579dc" },
});
...
const THEME_PRESETS = Object.freeze({
  default: Object.freeze({
    ...
    description: "A deep forest-night interface with mint light and warm Aniimo sparks.",
    motif: "signal",
    icon: "",
    colors: Object.freeze({
      background: "#0b1215",
      backgroundAlt: "#10242a",
      surface: "#121d21",
      surfaceRaised: "#1a2a2f",
      border: "#315057",
      text: "#f4f4e9",
      muted: "#9bb5b2",
      primary: "#62d9b3",
  ...
  meadow: Object.freeze({
    ...
    description: "The cream, mint, teal, and gold daytime interface.",
    motif: "signal",
    icon: "",
    colors: Object.freeze({
      background: "#f8f2d7",
      backgroundAlt: "#ccebe4",
      surface: "#fffaf0",
...
const els = {
  ...
  mapOverlayHeader: document.querySelector("#mapOverlayHeader"),
  mapOverlayBackButton: document.querySelector("#mapOverlayBackButton"),
  ...
  mapSectionLayer: document.querySelector("#mapSectionLayer"),
  mapSectionShortcutLayer: document.querySelector("#mapSectionShortcutLayer"),
...
function applyThemePreference() {
  ...
  const [red, green, blue] = hexColorRgb(colors.background);
  document.documentElement.style.colorScheme = ((red * 299 + green * 587 + blue * 114) / 1000) >= 150
    ? "light"
    : "dark";
...
function migrateChecklistCompletionIds(checklistData) {
  let changed = false;
  const entries = [
    ...(Array.isArray(checklistData?.entries) ? checklistData.entries : []),
    ...(Array.isArray(checklistData?.special_entries) ? checklistData.special_entries : []),
  ];
  entries.forEach((entry) => {
    (entry?.completion_states || []).forEach((completionState) => {
      const currentId = String(completionState?.id || "").trim();
      if (!currentId || state.completed.has(currentId)) return;
      const hasCompletedLegacyAlias = (completionState?.legacy_completion_ids || []).some((legacyId) => (
        state.completed.has(String(legacyId || "").trim())
      ));
      if (!hasCompletedLegacyAlias) return;
      state.completed.add(currentId);
      changed = true;
    });
  });
  if (changed) persistLocalTracking();
  return changed;
}
...
async function deleteCloudAccount() {
  ...
  try {
    ...
    console.error("Could not delete AniiLogs account", error);
    window.alert("Could not delete your account. Nothing was changed; please try again.");
  }
}
...
async function uploadCloudProgress() {
  if (!CLOUD_SYNC_ENABLED || !state.cloudSyncAuthenticated || state.cloudSyncHydrating) return;
  try {
    const response = await apiFetch("/progress", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ progress: cloudProgressPayload() }),
    });
    if (response.status === 401) clearAuthSession();
    else if (!response.ok) throw new Error(`Cloud progress returned ${response.status}`);
  } catch (error) {
    console.error("Could not sync AniiLogs progress", error);
  }
}

function scheduleCloudProgressUpload() {
  if (!CLOUD_SYNC_ENABLED || !state.cloudSyncAuthenticated || state.cloudSyncHydrating) return;
  window.clearTimeout(state.cloudSyncTimer);
  state.cloudSyncTimer = window.setTimeout(() => void uploadCloudProgress(), 750);
}
...
async function hydrateCloudProgress() {
  ...
  try {
    ...
    if (remote.progress) applyCloudProgress(remote.progress, state.localSnapshotPresent);
    persistLocalTracking({ sync: false });
  } catch (error) {
    console.error("Could not load AniiLogs cloud progress", error);
  } finally {
    state.cloudSyncHydrating = false;
  }
  if (state.cloudSyncAuthenticated) scheduleCloudProgressUpload();
}
...
function clearLocalTracking() {
  try {
    ...
    window.localStorage.removeItem(LEGACY_LOCAL_TRACKING_STORAGE_KEY);
    window.localStorage.removeItem(LEGACY_LOCAL_COMPLETION_STORAGE_KEY);
    window.localStorage.removeItem(LEGACY_LOCAL_PREFERENCES_STORAGE_KEY);
    state.localStorageError = "";
    state.localSnapshotPresent = false;
    scheduleCloudProgressUpload();
    return true;
  } catch (error) {
    updateLocalStorageError(error);
    return false;
...
function renderThemePreview(draft) {
  ...
  [THEME_PRESETS.emberpup, THEME_PRESETS.pawney].forEach((preset) => {
    ...
    image.src = contentUrl(preset.icon);
...
function renderSettings() {
  ...
  const cloudIdentity = document.createElement("div");
  cloudIdentity.className = "settings-account";
  ...
  privacyLink.className = "settings-link";
  privacyLink.href = "/privacy.html";
  privacyLink.textContent = "Privacy";
  cloudActions.append(privacyLink);
  if (state.cloudSyncAuthenticated) {
    const logout = document.createElement("button");
    logout.type = "button";
    logout.textContent = "Log out";
    logout.addEventListener("click", () => void logoutCloudAccount());
    const deleteAccount = document.createElement("button");
    deleteAccount.type = "button";
    deleteAccount.className = "settings-danger";
    deleteAccount.textContent = "Delete account";
    deleteAccount.addEventListener("click", () => void deleteCloudAccount());
    cloudActions.append(logout, deleteAccount);
  } else if (CLOUD_SYNC_ENABLED) {
    const signIn = document.createElement("a");
    signIn.className = "settings-link";
    signIn.href = `${API_URL}/auth/discord/start?return_to=${encodeURIComponent(signInReturnUrl())}`;
...
function compareAniilogEntries(left, right) {
  const surfaceDifference = (left?.surface_id === "collection" ? 1 : 0)
    - (right?.surface_id === "collection" ? 1 : 0);
  if (surfaceDifference !== 0) return surfaceDifference;
  const numberDifference = Number(left?.research_number) - Number(right?.research_number);
  if (numberDifference !== 0) return numberDifference;
  const formDifference = Number(left?.form_id) - Number(right?.form_id);
  if (numberDifference === 0 && Number.isFinite(formDifference) && formDifference !== 0) return formDifference;
...
function renderAniilogActiveRule(group, option) {
  ...
  if (option.meta?.icon) {
    ...
    icon.src = contentUrl(option.meta.icon);
    icon.alt = "";
    icon.setAttribute("aria-hidden", "true");
    button.append(icon);
  }
...
function renderCatalogCategoryToolbar(view) {
  ...
  if (view === "itemlog") {
    ...
    if (quickCategories.children.length) select.append(quickCategories);
  ...
  categories.forEach((category) => {
    ...
    if (category === "all") {
      ...
      icon.src = contentUrl(roleMeta.icon);
      icon.alt = "";
      icon.setAttribute("aria-hidden", "true");
...
function renderCatalogAbilitySection(title, abilities, emptyText = "No data available.") {
  ...
  orderedAbilities.forEach((ability) => {
    ...
    const renderVariant = () => {
      ...
      const iconSource = displayed.icon || ability.icon || "";
      if (iconSource) {
        icon.src = contentUrl(iconSource);
      } else {
      ...
      if (combat) {
        ...
        const addBadge = (label, type, color = "", iconSrc = "") => {
          ...
          if (iconSrc) {
            ...
            badgeIcon.src = contentUrl(iconSrc);
            badgeIcon.alt = "";
...
function getAniilogExpandedGroups() {
  ...
  let saved = [];
  try {
    const current = localStorage.getItem(ANIILOG_EXPANDED_GROUPS_STORAGE_KEY);
    const raw = current ?? localStorage.getItem(LEGACY_ANIILOG_EXPANDED_GROUPS_STORAGE_KEY);
    saved = JSON.parse(raw || "[]");
    if (current === null && raw !== null) {
      localStorage.setItem(ANIILOG_EXPANDED_GROUPS_STORAGE_KEY, JSON.stringify(saved));
    }
  } catch {
    saved = [];
  }
...
function renderCatalogPreview(options = {}) {
  ...
  if (view === "aniilog" && state.aniilogData?.review_notice) {
    ...
    notice.className = "catalog-review-notice";
    notice.textContent = state.aniilogData.review_notice;
    els.catalogPanel.append(notice);
  }
  ...
  if (view === "itemlog" && !state.itemlogData) {
    ...
    const message = document.createElement("p");
    message.className = "catalog-empty";
    message.textContent = "Loading packaged item definitions and current-build evidence.";
    els.catalogPanel.append(message);
...
function renderChecklist() {
  ...
  if (state.checklistData.review_notice) {
    const notice = document.createElement("p");
    notice.className = "checklist-review-notice";
    notice.textContent = state.checklistData.review_notice;
    els.checklistList.append(notice);
  }
...
function renderMapBase() {
  ...
  els.mapImage.src = contentUrl(map.image);
  els.mapImage.onerror = () => {
    ...
    els.mapImage.src = contentUrl(map.fallback_image);
  };
...
function renderMapTileDetail() {
  ...
  visibleTiles.forEach((tile) => {
    ...
    image.draggable = false;
    image.loading = "eager";
    image.src = contentUrl(tile.image);
    image.style.left = `${tile.left}px`;
    image.style.top = `${tile.top}px`;
    image.style.width = `${tile.width}px`;
    image.style.height = `${tile.height}px`;
...
async function loadRequestedShortShareSelection() {
  ...
  try {
    const response = await apiFetch(`/v1/shares/${REQUESTED_SHORT_SHARE_ID}`, {
...
async function copySharedPinLink() {
  ...
  updateSharePinsButton();
  try {
    const response = await apiFetch("/v1/shares", {
      method: "POST",
      headers: {
...
function pinSizeFor(spawn) {
  ...
  if (spawn.source_kind === "current_aniimo_scene_entity" && !spawn.icon) return 12;
  if (isEggMarker(spawn)) return baseSize + 2;
  if (spawn.marker_type === "underground_entrance") return MOBILE_LAYOUT_QUERY.matches ? 20 : 22;
...
function renderCanvasPins() {
  ...
  state.canvasEntries.forEach((entry) => {
    ...
    const item = state.data.itemsById.get(spawn.item_id);
    const defaultIconSource = spawn.icon || item?.icon;
    const hoverIconSource = spawn.hover_icon || item?.hover_icon;
...
function prepareData(data) {
  ...
  data.maps = (Array.isArray(data.maps) && data.maps.length ? data.maps : [data.map]).filter((map) => (
    map
    && map.visibility !== "withheld"
    && !HIDDEN_MAP_IDS.has(String(map.id || ""))
    && !String(map.id || "").startsWith("procedural-")
    && !["3004", "3005"].some((sceneId) => (map.scene_ids || []).map(String).includes(sceneId))
  ));
  if (!data.maps.length) throw new Error("No reviewed non-procedural maps are available.");
...
async function loadMapData(mapId, token) {
  ...
  try {
    const response = await fetch(contentUrl(map.data_url));
...
function updateMapOverlayState() {
  const map = currentMap();
  const parent = map?.parent_map_id ? state.data?.mapsById.get(map.parent_map_id) : null;
  const open = Boolean(parent);
  els.mapSurface.classList.toggle("has-map-overlay", open);
  els.mapOverlayHeader.hidden = !open;
  if (!open) {
    els.mapOverlayTitle.textContent = "";
    els.mapOverlayBackButton.dataset.parentMapId = "";
    return;
  }
  els.mapOverlayTitle.textContent = map.label;
  els.mapOverlayBackButton.textContent = `← Back to ${parent.label}`;
  els.mapOverlayBackButton.dataset.parentMapId = parent.id;
}

function openMapOverlay(mapId) {
  const map = state.data?.mapsById.get(mapId);
  if (!map?.parent_map_id) return;
  switchMap(mapId);
}
...
function updateMapMeta() {
  const map = currentMap();
  if (state.bootstrap?.data_quality?.provisional) {
    const build = state.bootstrap.data_quality.package_version || "current";
    const regionCount = state.data.maps.filter((candidate) => !candidate.parent_map_id && !candidate.layer).length;
    const currentNodeCount = Number(map.counts?.spawns || 0);
    const currentMapAniimoForms = Number(map.counts?.aniimo || 0);
    const currentChecklistEntries = Number(state.bootstrap.catalog_counts?.checklist_entries);
    const parts = [
      `Build ${build} map audit`,
      `${regionCount} current regions`,
      `${map.width} × ${map.height}`,
      currentNodeCount > 0 ? `${currentNodeCount} audited map nodes` : "No audited map nodes for this view",
      currentMapAniimoForms > 0 ? `${currentMapAniimoForms} current Aniimo forms on this map` : null,
      "Aniilog numbering withheld pending current-build verification",
      Number.isFinite(currentChecklistEntries) ? `${currentChecklistEntries} current checklist entries` : null,
      "current POIs, items, and exact Aniimo scene entities",
      "unresolved overlays withheld",
    ].filter(Boolean);
    if (state.loadingMapId === map.id) parts.push("Loading map");
    if (state.mapLoadError) parts.push("Map data unavailable");
...
function bindEvents() {
  ...
  els.mapOverlayBackButton.addEventListener("click", () => {
    const parentMapId = els.mapOverlayBackButton.dataset.parentMapId;
    if (parentMapId) switchMap(parentMapId);
  });
  ...
  els.workspaceTabs.addEventListener("keydown", (event) => {
    ...
    const tabs = [...els.workspaceTabs.querySelectorAll(".workspace-tab:not([disabled]):not([hidden])")];
...
async function init() {
  ...
  const checklistRequest = ENABLED_WORKSPACE_VIEWS.has("checklist")
    ? fetch(CHECKLIST_URL)
      .then(async (response) => {
        ...
        migrateChecklistCompletionIds(checklist);
  ...
  if (
    (Array.isArray(loaded.items) && loaded.items.length > 0)
    || (Array.isArray(loaded.spawns) && loaded.spawns.length > 0)
  ) {
    state.legacyDataset = {
